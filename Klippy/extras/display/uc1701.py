...
UseTextSize_en = 24 #Chinese size = 32*24
class DisplayBase:
    def __init__(self, io, columns=128, x_offset=0):
        ...
        if UseTextSize_en == 16:
            self.font = [self._swizzle_bits(bytearray(c))
                     for c in font8x14.VGA_FONT]
        else :
             self.font = [self._swizzle_16x24bits(bytearray(c))
                     for c in font8x14.ATK_XFONT24x24_en]   
        self.icons = {}
    def flush(self):
        # 找出帧缓冲区中的所有差异并将其发送到芯片
    ...
    def _swizzle_16x24bits(self, data):
        top1 = mid1 = bot1 = 0
        top2 = mid2 = bot2 = 0
        for row in range(8):
            spaced = (data[row] * 0x8040201008040201) & 0x8080808080808080
            ...
            spaced = (data[row+8] * 0x8040201008040201) & 0x8080808080808080
            mid1 |= spaced >> (7 - row)
            spaced = (data[row+16] * 0x8040201008040201) & 0x8080808080808080
            bot1 |= spaced >> (7 - row)

            spaced = (data[row+24] * 0x8040201008040201) & 0x8080808080808080
            top2 |= spaced >> (7 - row)
            spaced = (data[row+32] * 0x8040201008040201) & 0x8080808080808080
            mid2 |= spaced >> (7 - row)
            spaced = (data[row+40] * 0x8040201008040201) & 0x8080808080808080
            bot2 |= spaced >> (7 - row)


        bits_top1 = [(top1 >> s) & 0xff for s in range(0, 64, 8)]
        bits_top2 = [(top2 >> s) & 0xff for s in range(0, 64, 8)]
        
        bits_mid1 = [(mid1 >> s) & 0xff for s in range(0, 64, 8)]
        bits_mid2 = [(mid2 >> s) & 0xff for s in range(0, 64, 8)]
        
        bits_bot1 = [(bot1 >> s) & 0xff for s in range(0, 64, 8)]
        bits_bot2 = [(bot2 >> s) & 0xff for s in range(0, 64, 8)]

        bits_top = bits_top1 + bits_top2
        bits_mid = bits_mid1 + bits_mid2
        bits_bot = bits_bot1 + bits_bot2

        return (bytearray(bits_top), bytearray(bits_mid), bytearray(bits_bot))           
    ...
    def write_text(self, x, y, data):   
        if UseTextSize_en == 16 :
            ...
            pix_x = x * 12
            page_top = self.vram[y * 2 + 1]
            page_mid = self.vram[y * 2 + 2]
            page_bot = self.vram[y * 2 + 3]
            for c in bytearray(data):
                chr = c - 32
                bits_top,bits_mid,bits_bot = self.font[chr]
                page_top[pix_x:pix_x+16] = bits_top
                page_mid[pix_x:pix_x+16] = bits_mid
                page_bot[pix_x:pix_x+16] = bits_bot
                pix_x += 12
